...
- **불규칙한 대기시간**: 3-10분 사이 랜덤 대기로 서버 부하 방지
...
# 대기시간 조정 (5-15분)
python kospi200_daily_batch.py --min-delay 5 --max-delay 15
```
...
4. **대기**: 3-10분 불규칙 대기 후 다음 종목 처리

## 💡 예상 소요시간

- **KOSPI200 전체**: 약 10-35시간 (대기시간에 따라)
- **증분 업데이트**: 약 5-15시간
- **종목당 평균**: 약 3-7분 (수집 + 대기)
...
2. **장시간 실행**: 전체 작업 시 20시간 이상 소요될 수 있음
